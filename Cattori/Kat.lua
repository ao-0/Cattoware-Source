...
local ESPPage = Window:Server("KAT", "")

local MainTab = ESPPage:Channel("Main")
local GameHookTab = ESPPage:Channel("Game Hook")
...
MainTab:Toggle("Aimbot", false, function(Enable)
    KatSettings.Aimbot = Enable
end)

MainTab:Dropdown("Aim Options", {"Head", "HumanoidRootPart"}, function(Current)
    KatSettings.AimbotPosition = Current
end)

MainTab:Seperator()

GameHookTab:Toggle("Silent-Aim", false, function(bool)
    KatSettings.SilentAim = bool
end)

GameHookTab:Dropdown("Silent Aim Options",{"Head", "HumanoidRootPart"}, function(bool)
    KatSettings.SilentAimOptions = bool
end)

function ClosestPlayerToMouseHook2()
    if not KatSettings.Aimbot then return end
    local target = nil
    local dist = math.huge
    for i,v in pairs(game:GetService("Players"):GetPlayers()) do
            if v.Name ~= plr.Name then
                if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and KatSettings.Aimbot and v.Character:FindFirstChild("HumanoidRootPart") then
...
mt.__index = newClose(function(t, k)
    if not checkcaller() and t == mouse and tostring(k) == "X" and string.find(getfenv(2).script.Name, "Client") and getClosestPlayerToCursor() and KatSettings.SilentAim then
    	local closest = getClosestPlayerToCursor(oldIndex(t, k), oldIndex(t, "Y")).Character[KatSettings.SilentAimOptions]
    	local pos = currentCamera:WorldToScreenPoint(closest.Position)
    	return pos.X
    end
    if not checkcaller() and t == mouse and tostring(k) == "Y" and string.find(getfenv(2).script.Name, "Client") and getClosestPlayerToCursor() and KatSettings.SilentAim then
    	local closest = getClosestPlayerToCursor(oldIndex(t, "X"), oldIndex(t, k)).Character[KatSettings.SilentAimOptions]
    	local pos = currentCamera:WorldToScreenPoint(closest.Position)
    	return pos.Y
    end
    if t == mouse and tostring(k) == "Hit" and string.find(getfenv(2).script.Name, "Client") and getClosestPlayerToCursor() and KatSettings.SilentAim then
        return getClosestPlayerToCursor(mouse.X, mouse.Y).Character[KatSettings.SilentAimOptions].CFrame
    end
...
VisualsTab:Slider("Length", 1, 20, 12, function(value)
...
VisualsTab:Slider("Size", 100, 600, false, function(value)
    ESPSettings.FOVSize = value
end)

VisualsTab:Slider("Round Corner", 12, 100, false, function(value)
...
game:GetService("RunService").RenderStepped:Connect(function()
	if aiming then
		camera.CFrame = CFrame.new(camera.CFrame.Position,ClosestPlayerToMouseHook2().Character[KatSettings.AimbotPosition].Position)
	end
end)