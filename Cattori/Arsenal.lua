...
local ArsenalSettings = {
    Aimbot = false,
    AimbotTeamCheck = false,
    SilentAimToggle = false,
    SilentAimPosition = "Head",
    AimbotPosition = "Head",
    NoRecoilVar = false,
...
local Window = DiscordLib:Window("Cattori Hub")
local MainPage = Window:Server("Arsenal", "")

local MainTab = MainPage:Channel("Main")
local VisualsTab = MainPage:Channel("Visuals")
local GameHookTab = MainPage:Channel("Game Hooks")
local GunMods = MainPage:Channel("Gun Mods")
local HumanoidTab = MainPage:Channel("Humanoid")

MainTab:Toggle("Aimbot", false, function(Enable)
    ArsenalSettings.Aimbot = Enable
end)
MainTab:Toggle("Team Check", false, function(Enable)
    ArsenalSettings.AimbotTeamCheck = Enable
end)

MainTab:Dropdown("Aim Options",{"Head", "HumanoidRootPart"}, function(Current)
    ArsenalSettings.AimbotPosition = Current
end)

GameHookTab:Toggle("Silent Aim + Wallbang", false, function(Enable)
    ArsenalSettings.SilentAimToggle = Enable
end)

local SilentAimDrop = GameHookTab:Dropdown("Silent Aim Options",{"Head", "HumanoidRootPart"}, function(Current)
    ArsenalSettings.SilentAimPosition = Current
end)

GunMods:Toggle("No Recoil", false, function(Enable)
    ArsenalSettings.NoRecoilVar = Enable
end)

GunMods:Toggle("No Spread", false, function(Enable)
    ArsenalSettings.NoSpreadVar = Enable
end)

GunMods:Toggle("Always Auto", false, function(Enable)
    ArsenalSettings.AutomaticModeVar = Enable
end)

GunMods:Toggle("Fast Firing", false, function(Enable)
    ArsenalSettings.FastFiringVar = Enable
end)

function ClosestPlayerToMouseHook()
    if not ArsenalSettings.SilentAimToggle then return end
    local target = nil
...
function ClosestPlayerToMouseHook2()
    if not ArsenalSettings.Aimbot then return end
    local target = nil
    local dist = math.huge
    if ArsenalSettings.AimbotTeamCheck then
        for i,v in pairs(game:GetService("Players"):GetPlayers()) do
            if v.Name ~= plr.Name then
                if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and ArsenalSettings.Aimbot and v.Team ~= plr.Team and v.Character:FindFirstChild("HumanoidRootPart") then
                    local screenpoint = camera:WorldToScreenPoint(v.Character.HumanoidRootPart.Position)
                    local check = (Vector2.new(mouse.X,mouse.Y)-Vector2.new(screenpoint.X,screenpoint.Y)).magnitude
                    if check < dist then
                        target  = v
        ...
        for i,v in pairs(game:GetService("Players"):GetPlayers()) do
            if v.Name ~= plr.Name then
                if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and ArsenalSettings.Aimbot and v.Character:FindFirstChild("HumanoidRootPart") then
                    local screenpoint = camera:WorldToScreenPoint(v.Character.HumanoidRootPart.Position)
                    local check = (Vector2.new(mouse.X,mouse.Y)-Vector2.new(screenpoint.X,screenpoint.Y)).magnitude
                    if check < dist then
                        target  = v
...
mt.__namecall = function(self,...)
	...
	if tostring(self) == "HitPart" and tostring(method) == "FireServer" then
        if ArsenalSettings.SilentAimToggle then
            args[1] = ClosestPlayerToMouseHook().Character[ArsenalSettings.SilentAimPosition]
            args[2] = ClosestPlayerToMouseHook().Character[ArsenalSettings.SilentAimPosition].Position
        end
...
VisualsTab:Toggle("2D Box", false, function(Enable)
    ESPSettings.Box2DEnabled = Enable
end)

VisualsTab:Toggle("3D Box", false, function(Enable)
    ESPSettings.Box3DEnabled = Enable
end)

VisualsTab:Seperator()

VisualsTab:Toggle("Name", false, function(Enable)
    ESPSettings.NameEnabled = Enable
end)

VisualsTab:Toggle("Outline", false, function(Enable)
    ESPSettings.TextOutlineEnabled = Enable
end)


VisualsTab:Slider("Text Size", 10, 20, 15, function(value)
    ESPSettings.TextSize = value
end)

VisualsTab:Seperator()

VisualsTab:Toggle("Tracer", false, function(Enable)
...
VisualsTab:Toggle("View Tracer", false, function(Enable)
    ESPSettings.ViewTracerEnabled = Enable
end)

VisualsTab:Slider("Length", 1, 20, 14, function(value)
    ESPSettings.Length = value
end)

VisualsTab:Seperator()

VisualsTab:Toggle("Skeleton", false, function(Enable)
    ESPSettings.SkeletonESPEnabled = Enable
end)

VisualsTab:Seperator()

VisualsTab:Slider("Line Thickness", 1, 6, 1, function(value)
...
VisualsTab:Toggle("FOV Circle", false, function(Enable)
    ESPSettings.FOVEnabled = Enable
end)

VisualsTab:Slider("Size", 100, 600, 300, function(value)
    ESPSettings.FOVSize = value
end)

VisualsTab:Slider("Round Corner", 12, 100, 12, function(value)
        ...
        if Player.Character ~= nil and Player.Character:FindFirstChild("Humanoid") ~= nil and Player.Character:FindFirstChild("HumanoidRootPart") ~= nil and Player ~= plr and Player.Character.Humanoid.Health > 0 then
            ...
            if onScreen then
                if ESPSettings.Box2DEnabled then
                    ...
                    HealthBar.Size = Vector2.new(2, (HeadPosition.Y - LegPosition.Y) / (game:GetService("Players")[Player.Character.Name].NRPBS["MaxHealth"].Value / math.clamp(game:GetService("Players")[Player.Character.Name].NRPBS["Health"].Value, 0, game:GetService("Players")[Player.Character.Name].NRPBS:WaitForChild("MaxHealth").Value)))
                    HealthBar.Position = Vector2.new(Box.Position.X - 6, Box.Position.Y + (1 / HealthBar.Size.Y))
                    HealthBar.Color = Color3.fromRGB(255 - 255 / (game:GetService("Players")[Player.Character.Name].NRPBS["MaxHealth"].Value / game:GetService("Players")[Player.Character.Name].NRPBS["Health"].Value), 255 / (game:GetService("Players")[Player.Character.Name].NRPBS["MaxHealth"].Value / game:GetService("Players")[Player.Character.Name].NRPBS["Health"].Value), 0)
                ...
                if ESPSettings.NameEnabled then
                    Name.Text = tostring("["..math.floor(Distance).."] "..Player.Name.." ["..math.floor(game:GetService("Players")[Player.Character.Name].NRPBS["MaxHealth"].Value).."/"..math.floor(game:GetService("Players")[Player.Character.Name].NRPBS["Health"].Value).."]")
...
game:GetService("RunService").Stepped:Connect(function()
	if ArsenalSettings.FastFiringVar then
		env.DISABLED = false
		env.DISABLED2 = false
	end
	if ArsenalSettings.NoSpreadVar then
		env.currentspread = 0
	end
	if ArsenalSettings.NoRecoilVar then
		env.recoil = 0
	end
	if ArsenalSettings.AutomaticModeVar then
		env.mode = "automatic"
	end
end)
...
game:GetService("RunService").RenderStepped:Connect(function()
	if aiming then  
		camera.CFrame = CFrame.new(camera.CFrame.Position,ClosestPlayerToMouseHook2().Character[ArsenalSettings.AimbotPosition].Position)
	end
end)
