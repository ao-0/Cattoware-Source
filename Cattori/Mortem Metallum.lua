...
local CombatTab = ESPPage:Channel("Combat")
...
CombatTab:Toggle("Auto-Parry", false, function(bool)
    MortemMetallumSettings.AutoParry = bool
end)

CombatTab:Slider("Parry Distance", 8, 20, 12, function(t)
    MortemMetallumSettings.ParryDistance = t
end)

CombatTab:Seperator()

CombatTab:Toggle("Kill Aura", false, function(bool)
    MortemMetallumSettings.KillAura = bool
end)

CombatTab:Toggle("Rainbow Weapon", false, function(bool)
    MortemMetallumSettings.RainbowWeapon = bool
end)

CombatTab:Toggle("ForceField Weapon", false, function(bool)
    if bool then
        for i,v in pairs(game:GetService("Workspace").Explosions:GetDescendants()) do
            if v:IsA("Part") then
...
GameHookTab:Toggle("Silent-Aim", false, function(bool)
    MortemMetallumSettings.SilentAim = bool
end)

GameHookTab:Dropdown("Silent Aim Options",{"Head", "HumanoidRootPart"}, function(bool)
    MortemMetallumSettings.SilentAimOptions = bool
end)
...
local function ClosestPlayerToMouse()
    ...
    for i,v in pairs(game:GetService("Players"):GetPlayers()) do
        if v.Name ~= plr.Name then
            if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and v.Character:FindFirstChild("HumanoidRootPart") and MortemMetallumSettings.SilentAim then
...
mt.__namecall = function(self,...)
	...
	if tostring(self) == "mouse" and method == "FireServer" then
        if MortemMetallumSettings.SilentAim then
			args[1] = ClosestPlayerToMouse().Character[MortemMetallumSettings.SilentAimOptions].Position
        end
...
game:GetService("RunService").RenderStepped:Connect(function()
    if MortemMetallumSettings.KillAura then
    ...
    if MortemMetallumSettings.AutoParry then
        char = plr.Character
            ...
            if plrChar ~= char and not table.find(blacklist, plrChar.Name) then
                ...
                for _, anim in next, anims do
                    if table.find(animIds, anim.Animation.AnimationId) then
                        if (plrChar:FindFirstChild("HumanoidRootPart").Position - char:FindFirstChild("HumanoidRootPart").Position).Magnitude <= MortemMetallumSettings.ParryDistance and plrChar.Humanoid.Health > 0 and not plrChar.Humanoid.PlatformStand then
                            parry()
                            wait(1)
                        end
                    end
    ...
    if MortemMetallumSettings.RainbowWeapon then
        for i,v in pairs(game:GetService("Workspace").Explosions:GetDescendants()) do
            if v:IsA("Part") then
