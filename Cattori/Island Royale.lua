...
MainTab:Toggle("Aimbot", false, function(Enable)
    IslandRoyaleSettings.Aimbot = Enable
end)
MainTab:Toggle("Team Check", false, function(Enable)
    IslandRoyaleSettings.AimbotTeamCheck = Enable
end)

MainTab:Dropdown("Aim Options",{"Head", "HumanoidRootPart"}, function(Current)
    IslandRoyaleSettings.AimbotPosition = Current
end)

GameHookTab:Toggle("Silent Aim + Wallbang", false, function(Enable)
    IslandRoyaleSettings.SilentAimToggle = Enable
end)

local SilentAimDrop = GameHookTab:Dropdown("Silent Aim Options",{"Head", "HumanoidRootPart"}, function(Current)
    IslandRoyaleSettings.SilentAimPosition = Current
end)

function ClosestPlayerToMouseHook2()
    if not IslandRoyaleSettings.Aimbot then return end
    local target = nil
    local dist = math.huge
    if IslandRoyaleSettings.AimbotTeamCheck then
        for i,v in pairs(game:GetService("Players"):GetPlayers()) do
            if v.Name ~= plr.Name then
                if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and IslandRoyaleSettings.Aimbot and v.Team ~= plr.Team and v.Character:FindFirstChild("HumanoidRootPart") then
        ...
        for i,v in pairs(game:GetService("Players"):GetPlayers()) do
            if v.Name ~= plr.Name then
                if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and IslandRoyaleSettings.Aimbot and v.Character:FindFirstChild("HumanoidRootPart") then
                    local screenpoint = camera:WorldToScreenPoint(v.Character.HumanoidRootPart.Position)
                    local check = (Vector2.new(mouse.X,mouse.Y)-Vector2.new(screenpoint.X,screenpoint.Y)).magnitude
                    if check < dist then
                        target  = v
...
game:GetService("RunService").RenderStepped:Connect(function()
	if aiming then
		camera.CFrame = CFrame.new(camera.CFrame.Position,ClosestPlayerToMouseHook2().Character[IslandRoyaleSettings.AimbotPosition].Position)
	end
end)
